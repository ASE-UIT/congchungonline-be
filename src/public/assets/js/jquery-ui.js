...
(function (factory) {
  ...
  const effectsEffectScale = $.effects.define('scale', function (options, done) {
    ...
    const el = $(this);
    const { mode } = options;
    const percent = parseInt(options.percent, 10) || (parseInt(options.percent, 10) === 0 ? 0 : mode !== 'effect' ? 0 : 100);
  ...
  const effectsEffectSlide = $.effects.define('slide', 'show', function (options, done) {
    ...
    const ref = direction === 'up' || direction === 'down' ? 'top' : 'left';
    const positiveMotion = direction === 'up' || direction === 'left';
    const distance = options.distance || element[ref === 'top' ? 'outerHeight' : 'outerWidth'](true);
    const animation = {};
  ...
  const widgetsAccordion = $.widget('ui.accordion', {
    ...
    _animate(toShow, toHide, data) {
      ...
      const down = toShow.length && (!toHide.length || toShow.index() < toHide.index());
  ...
  const widgetsMenu = $.widget('ui.menu', {
      ...
      this._on({
        ...
        blur(event) {
          this._delay(function () {
            const notContained = !$.contains(this.element[0], $.ui.safeActiveElement(this.document[0]));
            if (notContained) {
    ...
    _destroy() {
      const items = this.element.find('.ui-menu-item').removeAttr('role aria-disabled');
      const submenus = items.children('.ui-menu-item-wrapper').removeUniqueId().removeAttr('tabIndex role aria-haspopup');
    ...
    expand(event) {
      const newItem = this.active && this.active.children('.ui-menu ').find(this.options.items).first();
      ...
      if (children.length) {
        ...
        $.each(['first', 'last'], function (index, value) {
          ...
          if (instance && that[`_${instance.widgetName}Options`]) {
            const options = that[`_${instance.widgetName}Options`](children.length === 1 ? 'only' : value);
  ...
  $.extend(Datepicker.prototype, {
    ...
    _generateHTML(inst) {
      ...
      const today = this._daylightSavingAdjust(new Date(tempDate.getFullYear(), tempDate.getMonth(), tempDate.getDate())); // clear time
      const isRTL = this._get(inst, 'isRTL');
    ...
    _generateMonthYearHeader(inst, drawMonth, drawYear, minDate, maxDate, secondary, monthNames, monthNamesShort) {
      let inMinYear;
      let inMaxYear;
      let month;
      let years;
  ...
  $.widget('ui.draggable', $.ui.mouse, {
    ...
    _getRelativeOffset() {
      ...
      return {
        ...
        left:
          p.left - (parseInt(this.helper.css('left'), 10) || 0) + (!scrollIsRootNode ? this.scrollParent.scrollLeft() : 0),
      };
    },
  ...
  $.ui.plugin.add('resizable', 'animate', {
    stop(event) {
      ...
      const left = parseFloat(that.element.css('left')) + (that.position.left - that.originalPosition.left) || null;
      const top = parseFloat(that.element.css('top')) + (that.position.top - that.originalPosition.top) || null;
  ...
  $.ui.plugin.add('resizable', 'alsoResize', {
    ...
    resize(event, ui) {
      ...
      $(o.alsoResize).each(function () {
        ...
        const style = {};
        const css = el.parents(ui.originalElement[0]).length ? ['width', 'height'] : ['width', 'height', 'top', 'left'];
...
*/
  ...
  const widgetsSelectmenu = $.widget('ui.selectmenu', [
    $.ui.formResetMixin,
    {
      version: '1.12.1',
      ...
      _drawButton() {
        let icon;
        const that = this;
        const item = this._parseOption(this.element.find('option:selected'), this.element[0].selectedIndex);
  ...
  $.widget('ui.spinner', {
    ...
    _increment(i) {
      const { incremental } = this.options;
